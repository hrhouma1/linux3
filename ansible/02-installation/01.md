Voici un **README** révisé avec une **table des matières** incluant des liens pour naviguer entre les sections et la possibilité de **retourner en haut** à chaque titre de niveau 1.
...
Pour
 ...
 vérifier que toutes les machines peuvent communiquer entre elles via l'interface **NAT Network (enp0s8)**, tu peux utiliser la commande **ping** depuis n'importe quelle machine :
...
- Appliquer les configurations Netplan sur Ubuntu :
  ```bash
  sudo netplan apply
  ```

- Redémarrer le service réseau sur CentOS :
  ```bash
  sudo systemctl restart network
  ```
...
Ce README inclut des liens pour faciliter la navigation entre les sections et des retours en haut à chaque titre principal. Si tu veux d'autres ajustements, n'hésite pas à me le dire !

Voici une version mise à jour du **README** qui inclut également les commandes pour mettre les interfaces réseau **en mode down (désactivé)** et **up (activé)**, ainsi que les liens pour naviguer entre les sections.
...
+---------------------------------------------------+
|                   Master Node                    |
|           master-ubuntu-server 10.0.2.10          |
...
|   10.0.2.12     | |   10.0.2.13     | |   10.0.2.14     | |    10.0.2.15     |
...
3. [Configuration de chaque machine (Master et Workers)](#3-configuration-de-chaque-machine-master-et-workers)
   - [Master Node](#master-node-ubuntu-server)
   ...
   - [Windows Desktop](#windows-desktop-windows-10)
4. [Connexion SSH](#4-connexion-ssh)
5. [Vérification de la connectivité](#5-vérification-de-la-connectivité)
...
# 3. Configuration de chaque machine (Master et Workers)
...
1. **Configurer l’interface réseau (Bridge Adapter)** :
   ...
   - Modifie l'adresse IP pour `Ethernet` (Bridge Adapter) :
     - IP : `192.168.2.115`
...
Parfois, il peut être nécessaire de désactiver (mettre en down) ou de réactiver (mettre en up) les interfaces réseau pour tester ou réappliquer les configurations. Voici les commandes pour Ubuntu et CentOS :
...
Ce README contient une navigation complète entre les sections avec des liens, ainsi que des commandes pour gérer les interfaces réseau en les activant ou les désactivant. Si tu souhaites d'autres ajustements, n'hésite pas à demander !
